...
local function createModel(opt)
   ...
   local function get_convs(nInputPlane, nOutputPlane, stride)
      local conv_params = {
      ...
      for i,v in ipairs(conv_params) do
         if i == 1 then
            ...
            convs:add(Convolution(nInputPlane,nOutputPlane,table.unpack(v)))
         else
            convs:add(SBatchNorm(nOutputPlane)):add(ReLU(true))
            if opt.dropout > 0 then
               convs:add(nn.Dropout(opt and opt.dropout or 0,nil,true))
            end
            convs:add(Convolution(nOutputPlane,nOutputPlane,table.unpack(v)))
         end
      end
      return convs
   end

   local function wide_basic(nInputPlane, nOutputPlane, stride, nSquash)
      ...
      local concats = nn.ConcatTable()
      for i = 1, nSquash do
         local convs = get_convs(nInputPlane, nOutputPlane, stride)
         concats:add(convs)
      end
      ...
      concats:add(shortcut)

      return block:add(concats):add(nn.CAddTable())
   end

   -- Stacking Residual Units on the same stage
   local function layer(block, nInputPlane, nOutputPlane, count, stride, nSquash)
      local s = nn.Sequential()
      s:add(block(nInputPlane, nOutputPlane, stride, 1))
      for i=2,count,nSquash do
         s:add(block(nOutputPlane, nOutputPlane, 1, math.min(nSquash, count-i+1)))
      end
      return s
   end
   ...
   elseif opt.dataset == 'cifar10' or opt.dataset == 'cifar100' then
      ...
      model:add(layer(wide_basic, nStages[1], nStages[2], n, 1, q)) -- Stage 1 (spatial size: 32x32)
      model:add(layer(wide_basic, nStages[2], nStages[3], n, 2, q)) -- Stage 2 (spatial size: 16x16)
