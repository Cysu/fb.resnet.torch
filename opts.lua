...
function M.parse(arg)
   ...
   cmd:option('-dataset',    'msceleb1m', 'Options: msceleb1m')
   ...
   if opt.dataset == 'msceleb1m' then
      ...
      if not paths.dirp(opt.data) then
         cmd:error('error: missing MS-Celeb-1M data directory')
      elseif not paths.dirp(trainDir) then
         cmd:error('error: MS-Celeb-1M missing `train` directory: ' .. trainDir)
      end
